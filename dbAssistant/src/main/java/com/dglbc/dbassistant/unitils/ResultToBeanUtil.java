...
//                columnNames[i] = rsmd.getColumnLabel(i + 1).toUpperCase();
            ...
            while (rs.next()) {
                ...
                for (int i = 0; i < counts; i++) {
                    ...
                    value = objectFilter(value, beanType);
...
//            m.invoke(t, rs.getObject(parmerName));
     ...
     * @throws Exception
     */
    public static void browseField (Object object) throws Exception {
...
//            m.invoke(t, rs.getObject(parmerName));
    ...
    public static Object objectFilter(Object value, Class<?> beanType) {
        if (value != null) {
            // 这里是获取数据库字段的类型
            Class<?> dbType = value.getClass();
            // 处理日期类型不匹配问题
            if (dbType == java.sql.Timestamp.class && beanType == Date.class) {
                // value = new
