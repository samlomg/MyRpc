...
public abstract class SQLAssistant<T> {
    ...
    public Expression update(T o, Where... wheres) throws IllegalAccessException, NoSuchFieldException {
        //首先获取class的属性 来生成sql语句
        Field[] fields = clazz.getDeclaredFields();
        Table table = new Table(clazz.getSimpleName(), "A");
        SQLHelper sqlHelper = new SQLHelper(table);
        ...
        if (wheres.length == 0) {
            sqlHelper.where(where().eq(table, "sequence", clazz.getField("sequence").get(o)));
        } else {
